...
export const DatePickComponent = ({input, label, meta: { touched, error, warning, dirty, submitFailed }, ...custom}) => {
  ...
  return (
    ...
    style={{ textAlign:'left' }}>
      <DateInput
          ...
          minDate='01-01-1900'
        />
        { (dirty || submitFailed) && error && <Label pointing color='red'>{error}</Label> }
     </Form.Field>
  )
}


export const InputComponent = ({input, label, meta: { touched, error, warning, dirty, submitFailed }, ...custom}) => (
  <Fragment>
    <Form.Field
    error={(dirty || submitFailed) && !!error}
    style={{ textAlign:'left' }}>
...
export const SearchComponent = ({input, results, handleSearchChange, handleSelectResult, label, meta: { touched, error, warning, dirty, submitFailed }, ...custom}) => {
  return (
      <Fragment>
        <Form.Field
        error={(dirty || submitFailed) && !!error}
