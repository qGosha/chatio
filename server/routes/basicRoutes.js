...
module.exports = (app, io) => {
    ...
    try {
      ...
      getIHaveDialogWith.forEach(i => {
        ...
        if (i.members) {
          ...
          if (i.deletionHistory.length) {
            const myDeletionHistory = i.deletionHistory.find(u =>
      ...
      Object.keys(messagesForEveryContact).forEach(id => {
        if (!messagesForEveryContact[id].length) {
          delete messagesForEveryContact[id]
          delete iHaveDialogWith[id]
        }
      })
  ...
  app.post(
    "/api/chat/deleteConversationForUser",
    loggedIn,
    async (req, res) => {
      const { id } = req.body
      if (!id) {
